...
5. Context is not shared across languages or model experiments. Series-wide consistency is achieved by importing an explicit context pack (`booktx context export-pack` / `import-pack`), not by sharing profile-local files.
