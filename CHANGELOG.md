...
- Added `booktx guide` command that shows the current lifecycle stage and the next human and agent actions
- Added `booktx judge prepare-grammar` to create and prepare a grammar-only revision profile
- Added `booktx glossary remove`, `reset`, `mandate`, and `audit` subcommands for binding terminology decisions

### Changed

- Changed `booktx status` output to include the current lifecycle stage and the next human and agent actions
- Added human-first CLI command catalog metadata (audience, runtime mode, write intent, examples) for `--help` output
- **BREAKING:** Removed the `translation` group alias and the `actor` and `harness` identity groups
- **BREAKING:** Removed the top-level `next` and `next-chapter` commands and the `whoami` alias under `identity`
- Documented the human-first CLI, the current data flow through `TranslationStoreV2`, and the profile boundary model
- Added back the `booktx model` group as an advanced feature and made guide module imports lazy

### Fixed

- Fixed mypy strict type errors in source analysis and termbase and added a `tests` mypy override
- Fixed source analysis to assign the no-action bucket with the generic single-token reason
...
- Added grammar correction judge profile with acceptance workflow, revision-focused selection mode, and agents.md generation for grammar profiles

### Fixed

- Fixed command boundary test violation in judge module import guard
- Fixed missing documentation links and Windows test compatibility
