...
def _render_isolated_body(
    *, profile: str, target_locale: str, quality_mode: str = "basic"
    ...
    return (
        ...
        "\n"
        "Before writing, read all three generated task files:\n"
        ...
        "- binding context, glossary, termbase, and protected names win;\n"
        "- concordance evidence is advisory and never approval;\n"
        "- batch related cues into one lookup and allow a second only for "
        "a concrete ambiguity.\n"
        "- recommend glossary usage notes or dialogue decisions, but never write "
        "policy without human approval;\n"
        "- quote validation counters exactly; do not infer "
        "chunks_checked == chunks_passed.\n"
        "\n"
        "When the user says `continue`:\n"
        "\n"
        "1. Run `booktx translate todo-list . --state open --json`.\n"
        "2. If the todo list is empty, report that there is no bounded run to "
        "continue and ask the user for a scope. Do not invent one.\n"
        "3. Select the one open todo and use its exact `todo_id` with "
        "`todo-status --todo-id` and `todo-resume --todo-id`.\n"
        "4. If the todo is complete, report completion and stop.\n"
        "5. If the todo is incomplete, run `booktx translate todo-resume . "
        "--todo-id TODO_ID --format block`.\n"
        "6. Read `tasks/TASK.agent.md` first. Consult the full immutable context "
        "snapshot only when the brief identifies an unresolved ambiguity.\n"
        "7. Translate only the generated ingest block.\n"
        "8. Always lint the completed block before the first insert with "
        "`booktx translate lint-block . --task-id TASK --file ingest/TASK.block.txt "
        f"--format block{quality_option}`. Lint is read-only.\n"
        "9. If lint fails, repair the same ingest file and rerun lint once.\n"
        "10. Submit it with the exact `booktx translate insert . ...` "
        "command printed by booktx.\n"
        "11. The todo submit/insert workflow performs the scoped chapter check "
        "before continuation and reports any blocker.\n"
        "12. If the same failure class remains after the lint repair retry, stop "
        "and report it instead of looping.\n"
        "13. If the installed booktx package fails to import in isolated mode, "
        "stop and report that the checkout must be fixed from project-root "
        "development mode.\n"
        "14. Query exact todo status and inspect `must_continue` / "
        "`next_safe_command`.\n"
        "15. Repeat status/resume in this same assistant turn while the todo is "
        "incomplete; a successful insert is never a stop condition.\n"
        "16. Stop only when the requested todo is complete or booktx reports a "
        "stop condition.\n"
        "\n"
        ...
        "\n"
        "- For a bounded todo, use the scoped check performed by the todo "
        "submission workflow and stop when that todo is complete. Do not run a "
        ...
        "- In isolated profile mode, do not use unzip, zip, grep over archives, "
        "temporary extraction directories, or direct archive traversal. Use "
        "booktx check --epub-output and booktx epub inspect/grep/extract-text.\n"
        "\n"
