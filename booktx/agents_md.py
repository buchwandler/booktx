...
def _render_isolated_body(*, profile: str, target_locale: str) -> str:
    return (
        ...
        "7. If insert rejects because of malformed block structure, quote "
        "preservation, inline XHTML, placeholders, or protected terms, run "
        "`booktx translate lint-block . --task-id TASK --file ingest/TASK.block.txt "
        "--format block`, repair the same ingest file, and retry once.\n"
        "8. If the same class of rejection repeats, stop and report it instead "
        "of looping.\n"
        "9. If the installed booktx package fails to import in isolated mode, "
        "stop and report that the checkout must be fixed from project-root "
        "development mode.\n"
        "10. Run the scoped check command printed by the todo workflow.\n"
        "11. Repeat status/resume only until the requested todo is complete "
